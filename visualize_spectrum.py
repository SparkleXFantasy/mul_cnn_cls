    ...
    if opt.directory:
        ...
        for file in tqdm(files):
            ...
            if freq_imgs is None:
                ...
                try:
                    assert(result['freq_hpf'].shape[0] == freq_imgs.shape[1] and result['freq_hpf'].shape[1] == freq_imgs.shape[2])
                    freq_imgs = np.concatenate([freq_imgs, np.expand_dims(result['freq_hpf'], axis=0)], axis=0)
                except:
                    print('Warning: Unmatched Shapes Found. Expected ({0}, {1}). Got ({2}, {3}). Ignore the Exception.'.format(
                        freq_imgs.shape[1], freq_imgs.shape[2], result['freq_hpf'].shape[0], result['freq_hpf'].shape[1]
                    ))
